...
sub validate_source {
    ...
    for my $Block ( sort keys %BlockCounter ) {
        if ( $BlockCounter{$Block} == 1 ) {
            $ErrorMessage .= "Block usage error. You used the block '$Block' only one time, the closing block is missing.\n";
        }
        if ( $BlockCounter{$Block} > 2 ) {
            $ErrorMessage .= "A block name should be unique. But you use the block '$Block'  for more than one time.\n";
        }
    }
